// Focused tests for bulk done / start / qa / edit command flows
...
describe('bulk done', () => {
    it('marks matching tasks as done when status filter is provided', async () => {
        ...
        // t3 is pending and must remain untouched
        expect(ctx.taskRepo.getById(t3.id)?.status).toBe('pending');
    });
...
describe('bulk start', () => {
    it('transitions matching tasks to in_progress', async () => {
        const restore = silenceConsole();
        const t1 = ctx.taskRepo.create({ title: 'Pending A', priority: 'low' });
        const t2 = ctx.taskRepo.create({ title: 'Pending B', priority: 'low' });
    ...
    it('does not affect tasks that do not match the given priority filter', async () => {
        ...
        expect(ctx.taskRepo.getById(high.id)?.status).toBe('pending');
    });
...
// ──────────────────────────────────────────────
describe('bulk qa', () => {
    it('transitions in_progress tasks to in_qa', async () => {
        const restore = silenceConsole();
        const task = ctx.taskRepo.create({ title: 'Review me', priority: 'medium' });
        ctx.taskRepo.update(task.id, { status: 'in_progress' });

        const { bulkCommand } = await import('../../src/commands/bulk.js');
        await bulkCommand.parseAsync(['qa', '--status', 'in_progress', '--yes'], { from: 'user' });

        restore();
        expect(ctx.taskRepo.getById(task.id)?.status).toBe('in_qa');
    });

    it('does not affect tasks not matching the filter', async () => {
        const restore = silenceConsole();
        const pending = ctx.taskRepo.create({ title: 'Pending task', priority: 'medium' });
        ...
        restore();
        // Only in_progress should become in_qa
        expect(ctx.taskRepo.getById(inProgress.id)?.status).toBe('in_qa');
        expect(ctx.taskRepo.getById(pending.id)?.status).toBe('pending');
    });
});
...
describe('bulk edit', () => {
    ...
    it('updates status via --set-status on all matching tasks', async () => {
        const restore = silenceConsole();
        const t1 = ctx.taskRepo.create({ title: 'Task X', priority: 'high' });

        const { bulkCommand } = await import('../../src/commands/bulk.js');
        await bulkCommand.parseAsync(
            ['edit', '--priority', 'high', '--set-status', 'in_progress', '--yes'],
            { from: 'user' },
        );
    ...
    it('routes --priority as edit target (alias for --set-priority) when used as the only filter source', async () => {
        const restore = silenceConsole();
        // Task already at 'low'; --priority low filters it AND sets it to low (idempotent alias behavior)
        const t1 = ctx.taskRepo.create({ title: 'Low task A', priority: 'low' });
        const t2 = ctx.taskRepo.create({ title: 'Low task B', priority: 'low' });

        const { bulkCommand } = await import('../../src/commands/bulk.js');
        // --priority low acts as both filter and edit; tasks at low are found and "set" to low
        await bulkCommand.parseAsync(
        ...
        restore();
        // Both tasks matched and were processed (priority stays low — idempotent)
    ...
    it('routes --status bare flag as edit target when --set-status is absent', async () => {
        const restore = silenceConsole();
        const t1 = ctx.taskRepo.create({ title: 'Pending task', priority: 'medium' });

        const { bulkCommand } = await import('../../src/commands/bulk.js');
        // --status pending filters by pending AND sets status to pending (idempotent)
        await bulkCommand.parseAsync(
            ['edit', '--status', 'pending', '--yes'],
            { from: 'user' },
        );

        restore();
        // Task matched and was processed
        expect(ctx.taskRepo.getById(t1.id)?.status).toBe('pending');
    });
